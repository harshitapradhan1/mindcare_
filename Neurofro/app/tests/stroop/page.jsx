...
export default function StroopTestGame() {
  ...
  const calculateAndSubmitResults = async () => {
    ...
    try {
      ...
      // Add accuracy and other fields from formData to the result
      setResult({
        ...data,
        accuracy: formData.accuracy,
        reaction_time: formData.reaction_time,
        // Ensure insights has severity if missing
        insights: data.insights ? {
          ...data.insights,
          severity: data.insights.severity || (data.label === 'Normal' ? 'low' : data.label === 'At Risk' ? 'medium' : 'high')
        } : null
      });
      ...
      try {
        ...
        if (userId) {
          const updateRes = await fetch(`${API_BASE}/neurotwin/update`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              user_id: userId,
          ...
          if (!updateRes.ok) {
            console.warn('NeuroTwin update failed:', updateRes.status, updateRes.statusText);
          }
        }
      ...
      setResult({ error: `Failed to submit results: ${error.message || 'Unknown error'}` });
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-teal-50 flex items-center justify-center p-4">
        <div className="max-w-3xl w-full">
          ...
          {result && !isSubmitting && !result.error && (
            <div className={`bg-gradient-to-br ${getSeverityConfig(result.insights?.severity || (result.label === 'Normal' ? 'low' : result.label === 'At Risk' ? 'medium' : 'high')).bgClass} border-2 ${getSeverityConfig(result.insights?.severity || (result.label === 'Normal' ? 'low' : result.label === 'At Risk' ? 'medium' : 'high')).borderClass} rounded-2xl p-8 mb-6 shadow-xl`}>
              <div className="flex items-center gap-4 mb-6">
                {getSeverityConfig(result.insights?.severity || (result.label === 'Normal' ? 'low' : result.label === 'At Risk' ? 'medium' : 'high')).icon}
              ...
              <div className="space-y-4">
                <div className="bg-white rounded-xl p-6 shadow-md border border-gray-200">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`px-5 py-2 rounded-full text-lg font-bold ${getSeverityConfig(result.insights?.severity || (result.label === 'Normal' ? 'low' : result.label === 'At Risk' ? 'medium' : 'high')).badge}`}>
                      {result.label || 'Unknown'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-700 font-bold text-lg">Confidence Level:</span>
                    <span className="text-2xl font-bold text-teal-700">{result.confidence ? (result.confidence * 100).toFixed(1) : '0.0'}%</span>
                  </div>
                </div>
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-md border border-gray-200">
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div>
                          ...
                          <p className="text-gray-700 leading-relaxed">{result.insights.message || 'No assessment message available.'}</p>
                        </div>
                      </div>
                    </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              onClick={() => window.location.href = '/tests/speech'}
